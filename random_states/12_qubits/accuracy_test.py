...
def main(training_data):
    ...
    for i in range(1,6,1):
        label_list += np.loadtxt(f'LABELS_200_Test_Set_{i}').tolist()
        gs_list += np.load(f'test_set_200examples_{i}.npy', allow_pickle=True).tolist()
    ...
    directory = f"real_labels_nqubits_{nqubits}/training_data{training_data}_"
    filename = "test_error"
    ...
    # Save the error value to the file
    np.savetxt(os.path.join(directory, filename), [test_error])
